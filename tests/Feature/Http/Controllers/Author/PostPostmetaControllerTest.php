...
use App\Http\Controllers\Author\PostPostmetaController;
...
it('renders create postmeta form', function() {
    $post = Post::factory()->hasPostmetas(3)->create();
    $this->get(action([PostPostmetaController::class, 'create'], ['post' => $post->slug]))->assertSee('Form for postmeta creation');
});
...
it('checks the validation and redirect', function() {
    ...
    $response->assertRedirect(route('author.posts.postmetas.index', ['post' => $post->slug]));
});
