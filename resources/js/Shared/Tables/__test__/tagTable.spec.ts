...
const paginatedData = {
    current_page: 1,
    data: [
        { id: 1, title: 'first_title', slug: 'slug', content: 'content', meta_title: 'meta' },
        { id: 2, title: 'first_title', slug: 'slug', content: 'content', meta_title: 'meta' },
    ],
...
beforeEach(async() => {
    wrapper = await mount(TagTable,{ global: {
...
describe('TagTable component', () => {
    ...
    it('checks the edit button events correcly hit the Inertia back-end', async () => {
        ...
        expect(Inertia.get).toHaveBeenCalledWith("/admin/tags/slug/edit")
    })
    ...
    it('checks the delete button events correcly hit the Inertia back-end', async () => {
        ...
        expect(Inertia.delete).toHaveBeenCalledWith("/admin/tags/slug", expect.anything())
    })
    ...
    it('checks the refresh button events correcly hit the Inertia back-end', async () => {
        ...
        expect(Inertia.get).toHaveBeenCalledTimes(1)
        expect(Inertia.get).toHaveBeenCalledWith("/admin/tags")
    })
    ...
    it('checks the add button events correcly hit the Inertia back-end', async () => {
        ...
        expect(Inertia.get).toHaveBeenCalledTimes(1)
        expect(Inertia.get).toHaveBeenCalledWith("/admin/tags/create")
    })
})
